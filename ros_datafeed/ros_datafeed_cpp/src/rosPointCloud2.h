...
#include "rosMsg.h"
#include "rosHeader.h"
#include "rosPointField.h"

namespace datafeed_sensor_msgs
{

  class PointCloud2 : public datafeed_ros::Msg
  {
    public:
      typedef datafeed_std_msgs::Header _header_type;
      _header_type header;
      ...
      typedef datafeed_sensor_msgs::PointField _fields_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(fields_lengthT > fields_length)
        this->fields = (datafeed_sensor_msgs::PointField*)realloc(this->fields, fields_lengthT * sizeof(datafeed_sensor_msgs::PointField));
      fields_length = fields_lengthT;
      for( uint32_t i = 0; i < fields_length; i++){
      offset += this->st_fields.deserialize(inbuffer + offset);
        memcpy( &(this->fields[i]), &(this->st_fields), sizeof(datafeed_sensor_msgs::PointField));
      }
      union {
        bool real;
        uint8_t base;
